...
async function syncSignInGoogle() {
  if (!_supabase) throw new Error('Supabase not initialized');
  // Save current local state so we can push it after OAuth redirect
...
function _syncRenderStep(step, errorMsg) {
  ...
  if (step === 'method') {
    body.innerHTML =
      ...
      '</button>' +
      '<div class="sm-divider"><span>' + (ko ? '또는' : 'or') + '</span></div>' +
...
async function _syncDoGoogle() {
  ...
  try {
    ...
    _syncRenderStep('method', (ko ? 'Google 로그인 실패: ' : 'Error: ') + e.message);
  }
}
